/**
 * Pre-PR review remediation at the @qontinui/ui-bridge-native invocation
 * seam (qontinui/ui-bridge#163).
 *
 * Plan: `2026-08-20-ui-bridge-action-declaration-shape.md`, Phases 2-3.
 *
 * This standalone package cannot import `@qontinui/ui-bridge` (an OPTIONAL
 * peer), so it carries its own executor, validator and abortable primitive.
 * An untested duplicate drifts, so the same three review findings are pinned
 * here as in the twin under `@qontinui/ui-bridge` `src/native`:
 *
