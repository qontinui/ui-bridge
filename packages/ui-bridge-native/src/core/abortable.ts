 ...
 *
 * KEEP IN SYNC with `@qontinui/ui-bridge` `src/core/abortable.ts`. The module
 * has no imports and touches no DOM API — only `AbortController` and
 * `setTimeout`, both present in React Native.
 */
