 ...
 * — the `@qontinui/ui-bridge-native` copy.
 *
 * This package cannot import `@qontinui/ui-bridge` (an OPTIONAL peer), so it
 * carries its own copy of the validator. A copy that is not tested is a copy
 * that drifts, and drift in THIS module is a cross-repo contract break: the
 * documented subset is what `qontinui-runner` writes schemas against. Same
 * cases, same hand-written literals, run against the native copy.
 *
