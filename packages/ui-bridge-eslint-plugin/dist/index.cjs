...
function getJSXTagName(opening) {
  if (opening.name.type === utils.AST_NODE_TYPES.JSXIdentifier) {
...
function isFlaggableComponentElement(node) {
  if (!node || node.type !== utils.AST_NODE_TYPES.JSXElement) {
    return false;
  }
...
function hasIRSiblingAnnotation(parent) {
  for (const child of parent.children) {
    if (child.type !== utils.AST_NODE_TYPES.JSXElement) continue;
...
function findEnclosingJSXElement(node) {
  ...
  while (current) {
    if (current.type === utils.AST_NODE_TYPES.JSXExpressionContainer) {
      sawExpressionContainer = true;
    }
    if (current.type === utils.AST_NODE_TYPES.JSXElement) {
      return sawExpressionContainer ? current : null;
    }
    if (current.type === utils.AST_NODE_TYPES.JSXFragment) {
