/**
 * Phase 2 — param validation at the `@qontinui/ui-bridge` NATIVE
 * action-invocation seam (`src/native/*`, the six published `./native/*`
 * subpath exports).
 *
 ...
 *
 * This subtree is EXCLUDED from `tsconfig.json` and built without DTS, so it
 * is type-checked by nothing. These tests are the only gate on the edits here.
 *
