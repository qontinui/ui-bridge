 ...
 * ---------------------------------------------------------------------------
 *
 * Like `core/abortable.ts`, this module imports nothing — which is what lets
 * `src/native/*` (a tree that takes no runtime dependency on `src/core/*`
 * because it bundles for React Native) share the one implementation instead of
 * growing another copy, and lets both `core/types.ts` and `diagnostics/index.ts`
 * import `ParamSchemaIssue` from here without creating a module cycle.
 */
